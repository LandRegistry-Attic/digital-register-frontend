...
class TestAuthentication:
    ...
    def test_sign_in_logs_user_in_when_authentication_successful(
            self, mock_authenticate, mock_login, mock_audit):
        username = 'username1'
        ...
        mock_audit.assert_called_once_with('User {} logged in'.format(username))
    ...
    def test_sign_out_logs_user_out_when_user_id_present(
            self, mock_logout_user, mock_current_user, mock_audit):

        user_id = 'user_id_123'
        mock_current_user.get_id.return_value = user_id
        ...
        mock_audit.assert_called_once_with('User {} logged out'.format(user_id))
